...
class Card:
    def __init__(self, rank, suit):
        ...
        if suit not in suits:
            raise TypeError(
                'Suit must be a string: "Spades", "Hearts", "Clubs", or "Diamonds".')
...
def newDeck():
    ...
    # So that the bottom of the list is the top of the deck, i.e. the Ace of Spades is drawn first by 'cardDeck.pop()'.
    cardDeck.reverse()


# To generate the deck at the start. Note that it is not shuffled at first.
newDeck()
...
def shuffleDeck():
    ...
    global cardDeck
    for i in range(0, 3):
        # Python's pseudorandom generator is slightly patterned unless shuffled multiple times.
        shuffle(cardDeck)
...
def newCard():
    ...
    print("The {card} has been generated.".format(
        card=str(randCard.cardName())))
    return randCard
...
def cardHelp():
    ...
    print('\n' + '=' * 72)
    print(
        '=' * 13 + " [brilliantlyInsane]'s Python Cards: Instructions " + '=' * 14)
    print('=' * 72 + '\n')
    ...
    print('—' * 72)
    print(
        'You can replace individual cards in a hand using <hand>[card #] = cards.draw().')
    print('However, lists in Python start FROM 0, not 1!')
    print(
        '"hand[1] = cards.draw()" will replace the SECOND card in your hand, not the first!\n')
